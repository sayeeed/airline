...
object NegotiationUtil {
  ...
  */
  ...
  def getToAirportRequirements(airline : Airline, newLink : Link, existingLinkOption : Option[Link], airlineLinks : List[Link]) = {
    ...
    if (airline.getReputation() < 50) {
      val requirementTotal = requirements.foldLeft(0.0)((sum, requirement) => sum + requirement.value)
      val adjustment = -1 * Math.pow(requirementTotal, 0.5) + 1
      if (adjustment < -1) {
        requirements.append(NegotiationRequirement(GATEWAY, adjustment, s"Startup Vigor – better at negotiation while under ${STARTUP_MAX_REPUTATION} reputation"))
      }
    }
